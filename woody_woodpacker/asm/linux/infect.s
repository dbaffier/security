...
[BITS 64]
    ...
    global text_infect:function
    global infect_size:data

segment .text
    infect_size dd end - text_infect
    
text_infect:                               ; Mach-O version
    push rdi
    push rsi
    push rax
    push rbx
...
decrypt:
    lea rdi, [rel woody]
    add rdi, [rel woody_size]
    mov rdx, qword [rel sectext_size]
    lea rsi, [rel text_infect]
    add rsi, qword [rel sectext_offset]
    call RC4

back:
	lea 	rax, [text_infect]
	add 	rax, [old_entryoff]
	mov 	[old_entryoff], rax
    ...
    pop rsi
    pop rdi
    jmp qword [rel old_entryoff]
    ; push qword [old_entryoff]
    ; nop
    ; ret

end:
    sectext_offset dq 0x0
    sectext_size dq 0x0
    old_entryoff dq 0x0
    woody_size dq 0x0
    woody db ''
