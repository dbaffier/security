...
void			file_info_64(void *file, int file_size)
{
	...
	printf(CO2);
	printf("addr\t\t magic\t\t bit\t endian\t\t cputype\t cpusubtype\t filetype\t ncmds\t sizecmds\t flags\n");
	printf(DEF);
	printf("%p\t %p\t %s\t %s\t %d\t\t %d\t\t %d\t\t %d\t %d\t\t %#.08x\n",
			  header, header->magic,
	...
	printf(CO3);
	printf("cmd\t\t cmdsize\t segname\t vmaddr\t\t vmsize\t fileoff\t filesize\t maxprot\t initprot\t nsects\t flags\n");
	printf(DEF);
	for (size_t i = 0, j = 0; i < header->ncmds; i++)
	{
		...
		if (cmd->cmd == LC_SEGMENT_64)
		{
			...
			printf("%-11s\t %#.5x\t %-11s\t %-11p\t %#x\t %#-5x\t\t %#-5x\t\t %d %s\t %d %s\t %-5d\t %#x\n",
			...
			if (segment->nsects)
			{
				...
				if (strcmp(section->sectname, SECT_TEXT) == 0)
					sectext = section;
				printf(CO4);
				printf("\t section\t segment\t addr\t\t size\t offset\t align\t reloff\t nreloc\t flags\t\t 1\t 2\t3\n");
				printf(DEF);
				for (j = 0; j < segment->nsects; j++) {
					printf("\t %-15s %-11s\t %-11p\t %#.5x %#.5x %#x\t %#x\t %#x\t %#.8x\t %#x\t %#x\t %#x\n",
		...
		else if (cmd->cmd == LC_DYSYMTAB)
		{
			...
			printf(CO3);
			printf("cmd\t\t cmdsize ilocalsym nlocalsym iextdefsym nextdefsym iundefsym nundefsym tocoff ntoc modtaboff nmodtab "
					  "extrefsymoff nextrefsyms\n");
			printf(DEF);
			printf("\t\t %#.12x %#.11x %#.9x %#.7x %#.9x %#.7x\n",
					  dsym->indirectsymoff, dsym->nindirectsyms, dsym->extreloff, dsym->nextrel, dsym->locreloff, dsym->nlocrel);
			printf(CO3);
			printf("\t\t indirectsymoff nindirectsyms extreloff nextrel locreloff nlocrel\n");
			printf(DEF);
		}
		else
		{
...
void			print_hex(u_char *file, size_t size)
{
	for (uint32_t i = 0; i < size; i++)
	{
		...
		if (i && ((i + 1) == size || ((i + 1) % 16) == 0))
		{
			...
			{
				printf("%c", ft_isprint(file[k]) ? file[k] : '.');
			}
		}
	}
	printf("\n");
