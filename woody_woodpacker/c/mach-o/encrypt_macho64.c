...
static void patch_file(t_packer *pack, struct section_64 *sectext, struct segment_command_64 *lastseg);
...
void encrypt_macho64(t_packer *pack)
{
    ...
    patch_file(pack, sectext, lastseg);
}
...
static void patch_file(t_packer *pack, struct section_64 *sectext, struct segment_command_64 *lastseg)
{
    ...
    size_t isize = infect_size + sizeof(sectext->size) + sizeof(uint64_t)
                    + pack->marksize + sizeof(pack->marksize) + sizeof(old_entryoff) + 16;
